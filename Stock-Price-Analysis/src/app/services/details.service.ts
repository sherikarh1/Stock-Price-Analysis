...
import { FutureChart, FutureChartResponse } from '../models/FutureChart';
...
export class DetailsService {
  ...
  public getPredictChart(ticker: string, duration: number) {
    return this.http.get<FutureChartResponse>('http://localhost:5000/predict?ticker=' + ticker + '&duration=' + duration)
    
    .pipe(
      tap( (res: FutureChartResponse) => {
        res.results = res.results.map(
          predictChart => new FutureChart(predictChart.date, predictChart.open));
  ...
  public getTopNews(ticker: string): Observable<NewsResponse> {

    return this.http.get<NewsResponse>(this.rootURL + '/news/'+ticker)
